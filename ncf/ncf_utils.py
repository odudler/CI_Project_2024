...
def create_train_eval_split(dataset, train_split=0.8, seed=42):
    ...
    print(sum(train_dataset.indices), sum(eval_dataset.indices))
...
def _create_interaction_matrix(num_users, num_items, dataset, user_or_item: Literal["user", "item"], type: Literal["default", "weighted"], rating: int | None = None):
    users = dataset.users
    items = dataset.items
